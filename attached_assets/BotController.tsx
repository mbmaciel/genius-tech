...
import { Play, StopCircle, Pause, Download, RefreshCw, Settings, ChevronDown } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import derivAPI from "@/lib/derivApi";
import botService, { LogEntry, TradeOption, TradeState } from "@/lib/botService";
...
export function BotController() {
  ...
  const initBot = () => {
    ...
    const simpleStrategy = (bot: typeof botService) => {
      ...
      if (randomValue < 0.3) {
        bot.buy('CALL', tradeOptions.amount, tradeOptions.duration, tradeOptions.durationUnit);
      } else if (randomValue > 0.7) {
        bot.buy('PUT', tradeOptions.amount, tradeOptions.duration, tradeOptions.durationUnit);
      }
    };
    ...
    <>
      <Card className="grid-card">
        <CardHeader className="p-4 border-b border-gray-800 flex flex-row justify-between items-center space-y-0">
          <CardTitle className="text-lg font-medium font-poppins">Controle do Bot</CardTitle>
        ...
        <Tabs defaultValue="config">
          <TabsList className="w-full bg-[#162746] border-b border-gray-800 rounded-none px-4">
            <TabsTrigger value="config" className="data-[state=active]:bg-[#1f3158] data-[state=active]:shadow-none">
              Configurações
            </TabsTrigger>
            <TabsTrigger value="stats" className="data-[state=active]:bg-[#1f3158] data-[state=active]:shadow-none">
              Estatísticas
            </TabsTrigger>
            <TabsTrigger value="logs" className="data-[state=active]:bg-[#1f3158] data-[state=active]:shadow-none">
              Logs
            </TabsTrigger>
          </TabsList>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => updateTradeOption('symbol', value)}
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => updateTradeOption('contractType', value)}
                    disabled={botState === 'running'}
                  >
                    <SelectTrigger className="w-full bg-[#0e1a33] border-gray-700">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateTradeOption('amount', Number(e.target.value))}
                    disabled={botState === 'running'}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex space-x-2">
                    <Input
                      ...
                      onChange={(e) => updateTradeOption('duration', Number(e.target.value))}
                      disabled={botState === 'running'}
                      className="bg-[#0e1a33] border-gray-700"
                    />
                    <Select 
                      value={tradeOptions.durationUnit} 
                      onValueChange={(value) => updateTradeOption('durationUnit', value)}
                      disabled={botState === 'running'}
                    >
              ...
              <div className="pt-4 border-t border-gray-800">
                ...
                <div className="bg-[#0e1a33] p-3 rounded-md">
                  <p className="text-sm text-[#8492b4]">
                    Estratégia Atual: <span className="text-white">RSI Crossover</span>
                  </p>
                  <p className="text-xs text-[#8492b4] mt-1">
                    Quando RSI cruza abaixo de 30, compra CALL. Quando cruza acima de 70, compra PUT.
                  </p>
            ...
            <TabsContent value="stats" className="m-0 p-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-[#1f3158] p-3 rounded-md">
                  ...
                  <p className="text-xl font-medium text-[#00e5b3]">{statistics.totalWins}</p>
                </div>
                <div className="bg-[#1f3158] p-3 rounded-md">
                  <p className="text-xs text-[#8492b4]">Perdas</p>
                  <p className="text-xl font-medium text-[#ff444f]">{statistics.totalLosses}</p>
                </div>
                <div className="bg-[#1f3158] p-3 rounded-md">
                  <p className="text-xs text-[#8492b4]">Taxa de Acerto</p>
                  <p className="text-xl font-medium">{statistics.winRate.toFixed(2)}%</p>
                </div>
              </div>
              ...
              <div className="mt-4 bg-[#1f3158] p-3 rounded-md">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <p className={`text-lg font-medium ${statistics.totalProfit >= 0 ? 'text-[#00e5b3]' : 'text-[#ff444f]'}`}>
                ...
                <div className="w-full bg-[#0e1a33] h-2 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min(Math.abs(statistics.totalProfit) / 100 * 100, 100)}%` }}
                  ></div>
                </div>
              </div>
                  ...
                  <div className="flex items-center">
                    <span 
                      className={`inline-block w-2 h-2 rounded-full mr-2 ${
                        botState === 'running' ? 'bg-[#00e5b3]' : 
                        botState === 'paused' ? 'bg-[#ffc107]' : 
                        botState === 'error' ? 'bg-[#ff444f]' : 'bg-gray-500'
                      }`}
                    ></span>
                    <p className="text-sm">
                      {botState === 'running' ? 'Em execução' :
                       botState === 'paused' ? 'Pausado' :
                       botState === 'stopped' ? 'Parado' :
                       botState === 'error' ? 'Erro' : 'Inativo'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="h-60 overflow-y-auto bg-[#0e1a33] p-2">
                {logs.length === 0 ? (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-[#8492b4] text-sm">Nenhum log disponível</p>
                  </div>
                ) : (

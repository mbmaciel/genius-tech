...
class SimpleBotService {
  ...
  constructor() {
    console.log(
      "[SIMPLEBOT] Inicializando serviço de bot de trading EXCLUSIVAMENTE real",
    );
    ...
    tradingWebSocket.setEvents({
      onStatusChange: (status) => {
        console.log(
          `[SIMPLEBOT] Status da conexão de trading atualizado: ${status}`,
        );
      },
      onError: (error) => {
    ...
    if (token) {
      console.log(
        "[SIMPLEBOT] Token OAuth encontrado para trading, verificando validade...",
      );
      ...
      tradingWebSocket
        ...
        .then((result) => {
          if (result.isValid) {
            console.log(
              `[SIMPLEBOT] Token OAuth é válido para a conta ${result.loginid}`,
            );
            this.token = token;
            ...
            tradingWebSocket
              ...
              .then(() => {
                console.log(
                  "[SIMPLEBOT] Conexão de trading estabelecida, autorizando...",
                );
                return tradingWebSocket.authorize(token);
              })
              .then((response) => {
                console.log(
                  "[SIMPLEBOT] Conexão de trading autorizada com sucesso:",
                  response.authorize?.loginid,
                );
              })
              .catch((err) => {
                console.error(
                  "[SIMPLEBOT] Erro ao conectar WebSocket de trading:",
                  err,
                );
              });
          } else {
      ...
      console.warn(
        "[SIMPLEBOT] Token OAuth não encontrado, operações reais estarão indisponíveis",
      );
    }
  }
  ...
  private async validateToken(token: string): Promise<boolean> {
    try {
      ...
      if (result.isValid) {
        console.log(
          `[SIMPLEBOT] Token OAuth válido para conta: ${result.loginid}`,
        );
        this.token = token;
        return true;
      } else {
  ...
  private processContractCompletion(contract: any): void {
    ...
    console.log(
      `[SIMPLEBOT] Contrato ${contract.contract_id} finalizado: ${isWin ? "GANHO" : "PERDA"} de ${profit.toFixed(2)}`,
    );
  ...
  public async start(): Promise<boolean> {
    console.log(
      "[SIMPLEBOT] Método start() chamado - APENAS OPERAÇÕES REAIS (sem simulação)",
    );
    ...
    try {
      ...
      if (!this.token) {
        console.error(
          "[SIMPLEBOT] Não foi possível iniciar: Token OAuth não encontrado",
        );
        this.emitEvent({
          type: "error",
          message:
            "Token OAuth não encontrado. Faça login com sua conta Deriv.",
        });
        this.status = "error";
        return false;
      }
  ...
  private async executeRealOperation(): Promise<boolean> {
    ...
    if (this.status !== "running") {
      console.log(
        "[SIMPLEBOT] Bot não está em status running, não vai executar operação",
      );
      return false;
    }
    ...
    try {
      ...
      if (!tradingWebSocket.isConnected()) {
        console.log(
          "[SIMPLEBOT] WebSocket de trading não está conectado, tentando reconectar...",
        );
        try {
          await tradingWebSocket.connect();
          console.log(
            "[SIMPLEBOT] Conexão WebSocket de trading estabelecida, autorizando...",
          );
          const authResponse = await tradingWebSocket.authorize(this.token!);

          if (authResponse.error) {
            console.error(
              "[SIMPLEBOT] Erro na autorização com WebSocket de trading:",
              authResponse.error,
            );
            this.emitEvent({
              type: "error",
          ...
          console.log(
            "[SIMPLEBOT] Autorização de trading bem-sucedida:",
            authResponse.authorize?.loginid,
          );
        } catch (connError) {
          console.error(
            "[SIMPLEBOT] Erro ao reconectar/autorizar WebSocket de trading:",
            connError,
          );
          this.emitEvent({
            type: "error",
        ...
        console.log(
          "[SIMPLEBOT] WebSocket de trading está conectado mas não autorizado, autorizando...",
        );
        try {
          const authResponse = await tradingWebSocket.authorize(this.token!);
          ...
          if (authResponse.error) {
            console.error(
              "[SIMPLEBOT] Erro na autorização de WebSocket já conectado:",
              authResponse.error,
            );
            this.emitEvent({
              type: "error",
          ...
          console.log(
            "[SIMPLEBOT] WebSocket de trading autorizado com sucesso:",
            authResponse.authorize?.loginid,
          );
        } catch (authError) {
          console.error(
            "[SIMPLEBOT] Erro ao autorizar conexão de trading existente:",
            authError,
          );
          return false;
        }
      ...
      const duration = 1; // 5 ticks de duração
      const prediction = this.settings.prediction || 5; // Previsão padrão: 5

      console.log(
        `[SIMPLEBOT] Solicitando proposta para ${contractType} com valor ${amount}`,
      );
      ...
      console.log(
        "[SIMPLEBOT] Enviando solicitação de proposta:",
        proposalRequest,
      );

      // Obter proposta de contrato
      const proposalResponse =
        await tradingWebSocket.sendRequest(proposalRequest);

      if (proposalResponse.error) {
        console.error(
          "[SIMPLEBOT] Erro na solicitação de proposta:",
      ...
      console.log(
        "[SIMPLEBOT] Proposta recebida com sucesso, ID:",
        proposal.id,
      );
      ...
      if (buyResponse.error) {
        console.error(
          "[SIMPLEBOT] Erro na compra do contrato:",
          buyResponse.error,
        );
        this.emitEvent({
          type: "error",
      ...
      if (!buy || !buy.contract_id) {
        console.error(
          "[SIMPLEBOT] Compra retornou resposta inválida ou sem contract_id",
        );
        this.scheduleNextOperation();
        return false;
      }
      ...
      console.log(
        "[SIMPLEBOT] Contrato comprado com sucesso, ID:",
        buy.contract_id,
      );
      ...
      console.error(
        "[SIMPLEBOT] Erro não tratado ao executar operação real:",
        error,
      );
      this.emitEvent({
        type: "error",
  ...
  private async monitorContract(contractId: string | number): Promise<void> {
    ...
    try {
      ...
      if (response.error) {
        console.error(
          `[SIMPLEBOT] Erro ao monitorar contrato ${contractId}:`,
          response.error,
        );
      } else {
      ...
      console.error(
        `[SIMPLEBOT] Exceção ao monitorar contrato ${contractId}:`,
        error,
      );
    }
  }
  ...
  private simulateOperation(): void {
    console.log(
      "[SIMPLEBOT] Método simulateOperation() redirecionando para operação REAL",
    );
    this.executeRealOperation();
  }
}

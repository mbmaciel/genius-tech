...
class OAuthDirectService implements OAuthDirectServiceInterface {
    ...
    if (isWin) {
      ...
      if (consecutiveLosses >= configuracoes.usarMartingaleAposXLoss) {
        ...
        if (this.strategyConfig.toLowerCase().includes("ironunder")) {
          ...
          nextAmount =
            Math.round(buyPrice * (1 + configuracoes.martingale) * 100) / 100;
          console.log(
  ...
  private validateOperationContinuation(
    ...
    if (hasProfitTarget && this.sessionStats.netProfit >= profitTargetNum) {
      ...
      console.log(
        `[OAUTH_DIRECT] 🎯 META DE LUCRO ATINGIDA: ${this.sessionStats.netProfit.toFixed(2)} / ${profitTargetNum}`,
      );
    ...
    if (hasLossLimit && this.sessionStats.totalLoss >= lossLimitNum) {
      ...
      console.log(
        `[OAUTH_DIRECT] ⚠️ LIMITE DE PERDA ATINGIDO: ${this.sessionStats.totalLoss.toFixed(2)} / ${lossLimitNum}`,
      );

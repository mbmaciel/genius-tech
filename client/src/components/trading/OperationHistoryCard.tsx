...
export function OperationHistoryCard({
  ...
  console.log(
    "[OperationHistoryCard] Renderizando com operações:",
    operations.length,
    operations,
  );
  ...
  const loadSavedOperations = React.useCallback(() => {
    try {
      ...
      if (savedOperations) {
        ...
        console.log(
          "[OperationHistoryCard] 📂 Carregadas",
          processed.length,
          "operações salvas do localStorage",
        );
        return processed;
      }
    } catch (error) {
      console.error(
        "[OperationHistoryCard] Erro ao carregar operações salvas:",
        error,
      );
    }
    return [];
  ...
  const [internalOperations, setInternalOperations] = React.useState<
    Operation[]
    ...
    if (internalOperations && internalOperations.length > 0) {
      try {
        localStorage.setItem(
          "operation_history",
          JSON.stringify(internalOperations),
        );
        console.log(
          "[OperationHistoryCard] ✅ Histórico de",
          internalOperations.length,
          "operações salvo no localStorage",
        );
      } catch (error) {
        console.error(
          "[OperationHistoryCard] Erro ao salvar histórico no localStorage:",
          error,
        );
      }
    }
  ...
  React.useEffect(() => {
    console.log(
      "[OperationHistoryCard] Configurando auto-refresh para histórico de operações",
    );

    // Definir intervalo de atualização a cada 5 segundos
    const refreshInterval = setInterval(() => {
      console.log(
        "[OperationHistoryCard] Auto-refresh disparado, verificando atualizações...",
      );
  ...
  // ★★★ MODIFICAÇÃO CRÍTICA: SEMPRE ATUALIZAR O HISTÓRICO EM TEMPO REAL ★★★
  // Garantir que todas as operações sejam exibidas e nunca perder histórico
  React.useEffect(() => {
    console.log(
      "[OperationHistoryCard] 📊 Recebidas operações externas:",
      operations?.length || 0,
      operations,
    );
    ...
    if (operations && operations.length > 0) {
      ...
      setInternalOperations((prevOps) => {
        ...
        if (!prevOps || prevOps.length === 0) {
          console.log(
            "[OperationHistoryCard] Sem operações anteriores, usando operações recebidas:",
            operations.length,
          );
          return operations;
        }
  ...
  React.useEffect(() => {
    ...
    console.log(
      "[OperationHistoryCard] Sistema simplificado de atualização ativado",
    );

    // Exibir informações detalhadas das operações no histórico quando o componente é montado
    console.log(
      "[OperationHistoryCard] ★★★ INFORMAÇÕES DETALHADAS DO HISTÓRICO ★★★",
    );
    console.log(
      "[OperationHistoryCard] Quantidade de operações:",
      internalOperations.length,
    );
    ...
    return () => {
      console.log(
        "[OperationHistoryCard] Sistema simplificado de atualização desativado",
      );
    };
  }, [internalOperations]);
  ...
  const safeOperations = Array.isArray(internalOperations)
    ? internalOperations
    : [];
  console.log(
    "[OperationHistoryCard] 🔍 DIAGNÓSTICO: safeOperations.length =",
    safeOperations.length,
  );

  // ★★★ CORREÇÃO CRÍTICA: Garantir que temos arrays válidos e separação correta ★★★
  console.log(
    "[OperationHistoryCard] 🔍 DIAGNOSTICANDO separação de operações...",
  );
  ...
  const intermediateOperations = safeOperations.filter((op) => {
    ...
    console.log(
      `[OperationHistoryCard] Operação ${op.id}: isIntermediate=${isInterm}`,
    );
    return isInterm;
  });
  ...
  console.log(
    "[OperationHistoryCard] - Total de operações:",
    safeOperations.length,
  );
  console.log(
    "[OperationHistoryCard] - Operações na aba principal:",
    regularOperations.length,
  );
  console.log(
    "[OperationHistoryCard] - Operações intermediárias:",
    intermediateOperations.length,
  );
            ...
            <div className="overflow-y-auto max-h-[240px] scrollbar-thin">
              {regularOperations.length > 0 ? (
                    ...
                    return (
                      <div
                        ...
                        className={`p-2 rounded-md border flex justify-between items-center ${
                          isWin
                            ? "bg-[#1a2d2a] border-[#2a5a4a]"
                            : "bg-[#2d1a1a] border-[#5a2a2a]"
                        }`}
                      >
                        ...
                        <div className="flex items-center">
                          ...
                          <div
                            className={`flex items-center ${isWin ? "text-green-500" : "text-red-500"}`}
                          >
                            {isWin ? (
                ...
                <div className="text-center py-4 text-gray-400">
                  Nenhuma operação registrada. Inicie o robô para começar a
                  operar.
                </div>
              )}
            </div>
          ...
          <TabsContent value="intermediate" className="mt-0">
            <div className="overflow-y-auto max-h-[240px] scrollbar-thin">
              {intermediateOperations.length > 0 ? (
                ...
                <div className="text-center py-4 text-gray-400">
                  Nenhuma análise Advance disponível. Utilize a estratégia
                  Advance para ver análises detalhadas aqui.
                </div>
              )}
            </div>
          </TabsContent>

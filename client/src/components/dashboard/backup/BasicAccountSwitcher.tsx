...
export function BasicAccountSwitcher() {
  ...
  const handleAccountClick = (account: Account) => {
    ...
    // Pedir confirmação do usuário usando confirm nativo do navegador
    const confirmSwitch = window.confirm(
      `Deseja trocar para a conta ${account.loginid}?\n\n` +
      "Essa ação recarregará a página e definirá a nova conta como principal."
    );
  ...
  const switchAccount = async (account: Account) => {
    try {
      toast({
        ...
        description: `Preparando para trocar para conta ${account.loginid}...`,
        variant: "default",
      });

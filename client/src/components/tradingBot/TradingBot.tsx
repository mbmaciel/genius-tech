...
import { Bot, Play, StopCircle, AlertCircle, FileText, BarChart2, Settings } from 'lucide-react';
...
export default function TradingBot({ apiToken, isConnected, onError }: TradingBotProps) {
  // Strategy state
  ...
  useEffect(() => {
    const fetchSymbols = async () => {
      ...
      try {
        ...
        const volatilitySymbols = symbols.filter(s => 
  ...
  const handleStartTrading = async () => {
    if (!isConnected) {
      toast({
        ...
        description: "Você precisa estar conectado à API Deriv para iniciar operações.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      addLog(`Inscrito com sucesso para receber ticks de ${selectedSymbol}`, 'success');
      
      // Set simulated data for UI state
      localStorage.setItem('trading_bot_running', 'true');
      localStorage.setItem('trading_bot_data', JSON.stringify({
      ...
      toast({
        title: "Erro",
        description: "Ocorreu um erro ao iniciar as operações. Verifique o log para mais detalhes.",
        variant: "destructive",
      });
    } finally {
      setIsBusy(false);
  ...
  const simulateTrade = () => {
    ...
    if (isWin) {
      setWins(prev => prev + 1);
      addLog(`Operação #${contractResults.length + 1} encerrada com LUCRO: $${profit.toFixed(2)}`, 'success');
    } else {
      setLosses(prev => prev + 1);
      addLog(`Operação #${contractResults.length + 1} encerrada com PREJUÍZO: $${Math.abs(profit).toFixed(2)}`, 'error');
    }
    
    // Update localStorage data
    const totalTrades = wins + losses + 1;
    const winRate = totalTrades > 0 ? ((isWin ? wins + 1 : wins) / totalTrades) * 100 : 0;
    ...
    localStorage.setItem('trading_bot_data', JSON.stringify({
      ...
      startTime: JSON.parse(localStorage.getItem('trading_bot_data') || '{}').startTime || Date.now()
    }));
        ...
        <CardContent className="p-0 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="col-span-1">
              <Label className="text-sm font-medium text-[#8492b4] mb-2 block">Selecione a Estratégia</Label>
              <Select
            ...
            <div className="col-span-1">
              <Label className="text-sm font-medium text-[#8492b4] mb-2 block">Valor Inicial</Label>
            ...
            <div className="col-span-1">
              <Label className="text-sm font-medium text-[#8492b4] mb-2 block">Stop Loss</Label>
              <div className="relative">
                <Input
                  type="number"
                  value={stopLoss}
          ...
          <div className="bg-[#0e1a33]/40 rounded-lg p-4 mb-6 border border-[#1c3654]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Martingale</Label>
                <Select
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Máx. Martingale</Label>
                <Input
                  type="number"
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Símbolo</Label>
                <Select
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Take Profit</Label>
                <Input
                  type="number"
                  value={takeProfit}
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Stop Consecutivas</Label>
                <Input
                  type="number"
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Temporalidade</Label>
                <Select
                  value={timeframe}
              ...
              <div>
                <Label className="text-[#8492b4] text-xs mb-1 block">Operações</Label>
          ...
          <Tabs defaultValue="results" className="w-full">
            <TabsList className="bg-[#1f3158] text-white mb-4">
              <TabsTrigger value="results" className="data-[state=active]:bg-[#0e1a33]">
                <BarChart2 className="h-4 w-4 mr-2" />
                Resultados
              </TabsTrigger>
              <TabsTrigger value="log" className="data-[state=active]:bg-[#0e1a33]">
            ...
            <TabsContent value="results">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-[#0e1a33]/40 rounded-lg p-4 border border-[#1c3654]">
                  ...
                  <div className={`text-2xl font-bold ${totalProfit >= 0 ? 'text-[#10b981]' : 'text-[#f43f5e]'}`}>
                  ...
                  {contractResults.length > 0 ? (
                    <table className="w-full text-sm">
                      <thead className="bg-[#1f3158]">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">#</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Tipo</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Resultado</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Lucro/Prejuízo</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Entrada</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Saída</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-[#8492b4]">Hora</th>
                        </tr>
                      </thead>
                      <tbody>
                        {contractResults.map((contract, index) => (
                          <tr key={index} className={index % 2 === 0 ? 'bg-[#0e1a33]/20' : 'bg-[#0e1a33]/40'}>
                            <td className="px-4 py-2 text-white">{contract.id}</td>
                            <td className="px-4 py-2 text-white">{contract.type}</td>
                            <td className="px-4 py-2">
                              <span className={`px-2 py-1 rounded text-xs font-medium ${
                                ...
                                {contract.result === 'won' ? 'Ganhou' : contract.result === 'lost' ? 'Perdeu' : 'Pendente'}
                              </span>
                            </td>
                            <td className={`px-4 py-2 font-medium ${contract.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                              {contract.profit >= 0 ? '+' : ''}{contract.profit.toFixed(2)} USD
                            </td>
                            <td className="px-4 py-2 text-white">{contract.entry?.toFixed(2)}</td>
                            <td className="px-4 py-2 text-white">{contract.exit?.toFixed(2) || '-'}</td>
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  {logEntries.length > 0 ? (
                    ...
                    <div className="text-[#8492b4]">Sistema aguardando início das operações...</div>
                  )}
                </div>
              </div>
            </TabsContent>
